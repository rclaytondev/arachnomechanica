...
export class StoneTile extends BasicTile {
    ...
    static initializeLines() {
        ...
        for (const point of points) {
            ...
            for (const otherPoint of others.slice(0, WorldData.STONE_CONNECTIONS)) {
                const closestX = ArrayUtils.minValue([otherPoint.x, otherPoint.x - WorldData.STONE_PATTERN_WIDTH, otherPoint.x + WorldData.STONE_PATTERN_WIDTH], x => MathUtils.dist(x, point.x));
                const closestY = ArrayUtils.minValue([otherPoint.y, otherPoint.y - WorldData.STONE_PATTERN_HEIGHT, otherPoint.y + WorldData.STONE_PATTERN_HEIGHT], y => MathUtils.dist(y, point.y));
                lines.push({ point1: point, point2: new Vector(closestX, closestY) });
            }
        }
        return lines;
    }
    static linesImage = null;
    static initializeLinesImage() {
        ...
        for (const { point1, point2 } of lines) {
            canvasIO.pointedLine(point1.x - box.left, point1.y - box.top, point2.x - box.left, point2.y - box.top);
        }
        StoneTile.linesImage = canvasIO.canvas;
        return canvasIO.canvas;
    }
    static displayStoneTiles(world, canvasIO, visibleTileRegion) {
        canvasIO.ctx.save();
        ...
        canvasIO.ctx.clip();
        const patternRegion = visibleTileRegion.scale(WorldData.TILE_SIZE / WorldData.STONE_PATTERN_WIDTH, WorldData.TILE_SIZE / WorldData.STONE_PATTERN_HEIGHT);
        for (let x = Math.floor(patternRegion.left - 1); x < patternRegion.right; x++) {
            for (let y = Math.floor(patternRegion.top - 1); y < patternRegion.bottom; y++) {
                canvasIO.ctx.drawImage(StoneTile.initializeLinesImage(), x * WorldData.STONE_PATTERN_WIDTH, y * WorldData.STONE_PATTERN_HEIGHT);
            }
        }
...
//# sourceMappingURL=StoneTile.mjs.map