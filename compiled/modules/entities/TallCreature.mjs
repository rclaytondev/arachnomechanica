...
class TallCreatureLeg {
    ...
    offsetX;
    offsetDirection;
    constructor(position, attachmentX, offsetX, offsetDirection) {
        this.position = position;
        this.attachmentX = attachmentX;
        this.offsetX = offsetX;
        this.offsetDirection = offsetDirection;
    }
    static initialize(tallCreature, attachmentX, offsetDirection, world) {
    ...
    attachmentPoint(tallCreature) {
        return tallCreature.head.edgeCenter("down").add(new Vector(this.attachmentX, 0));
    }
    destination(tallCreature, world) {
        const attachmentPoint = this.attachmentPoint(tallCreature);
        const distance = world.lineIntersectionDistance(attachmentPoint, Vector.unit("down"), TallCreatureData.MAX_LEG_HEIGHT, [], (e) => e !== tallCreature);
        return attachmentPoint.add(this.offsetX, distance);
    }
    update(tallCreature, world) {
        this.updatePosition(tallCreature, world);
        this.updateOffset();
    }
    updatePosition(tallCreature, world) {
...
class TallCreatureStabber {
    ...
    update(tallCreature, world, canvasIO) {
    ...
    checkStabbing(tallCreature, world) {
        const yIntersects = (world.player.hitbox.bottom > tallCreature.head.top &&
            world.player.hitbox.top < tallCreature.head.bottom);
        if (!yIntersects) {
            return;
        }
        const distance = (this.direction === "left"
    ...
    setExtension(amount, tallCreature, world, canvasIO) {
...
export class TallCreature extends Collideable {
    ...
    constructor(position, legHeight, world) {
        super();
    ...
    displayGlowEffect(canvasIO) {
        const center = this.head.center();
        GameUtils.glowCircle(center.x, center.y, TallCreatureData.GLOW_SIZE, TallCreatureData.GLOW_INTENSITY, canvasIO, TallCreatureData.EYE_COLOR.red, TallCreatureData.EYE_COLOR.green, TallCreatureData.EYE_COLOR.blue);
    }
    ...
    legsHitbox() {
        ...
        return Rectangle.fromDimensions(center - TallCreatureData.LEG_HITBOX_WIDTH / 2, this.head.bottom, TallCreatureData.LEG_HITBOX_WIDTH, this.legHeight);
    }
    hitboxes() {
        return [
            this.head,
...
//# sourceMappingURL=TallCreature.mjs.map