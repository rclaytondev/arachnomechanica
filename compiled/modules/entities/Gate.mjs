...
export class Gate extends RectangularCollideable {
    direction; // which way the gate moves when closing
    playerSide = "positive";
    toggled = false;
    lastFrameUpdated = -Infinity;
    openness;
    constructor(hitbox, direction, toggled) {
        super(hitbox);
        this.direction = direction;
        this.toggled = toggled;
        this.openness = toggled ? 0 : 1;
    }
    static atTile(tilePosition, direction, toggled) {
    ...
    opennessTarget(gateController) {
        return this.toggled ? 1 - gateController.openness : gateController.openness;
    }
    static getPhysicsBox(tilePosition, direction, closedness) {
        if (direction === "down") {
            return Rectangle.fromDimensions(tilePosition.x * WorldData.TILE_SIZE, tilePosition.y * WorldData.TILE_SIZE, WorldData.TILE_SIZE, closedness * WorldData.TILE_SIZE);
        }
        else if (direction === "up") {
            return Rectangle.fromDimensions(tilePosition.x * WorldData.TILE_SIZE, (tilePosition.y + 1 - closedness) * WorldData.TILE_SIZE, WorldData.TILE_SIZE, closedness * WorldData.TILE_SIZE);
        }
        else if (direction === "left") {
            return Rectangle.fromDimensions((tilePosition.x + 1 - closedness) * WorldData.TILE_SIZE, tilePosition.y * WorldData.TILE_SIZE, closedness * WorldData.TILE_SIZE, WorldData.TILE_SIZE);
        }
        else {
            return Rectangle.fromDimensions(tilePosition.x * WorldData.TILE_SIZE, tilePosition.y * WorldData.TILE_SIZE, closedness * WorldData.TILE_SIZE, WorldData.TILE_SIZE);
        }
    }
    render() {
    ...
    display(canvasIO) {
        ...
        for (const size of [WorldData.TILE_ACCENT_INSET, GateData.INNER_ACCENT_INSET]) {
            canvasIO.strokeSquare(patternBox.x + size, patternBox.y + size, WorldData.TILE_SIZE - 2 * size);
        }
        const center = patternBox.center();
        const directions = (Directions.isHorizontal(this.direction) ? ["left", "right"] : ["up", "down"]);
        for (const direction of directions) {
            canvasIO.ctx.save();
            canvasIO.ctx.translate(center.x, center.y);
            canvasIO.rotateTo("down", direction);
            canvasIO.strokeLine(0, GateData.INNER_ACCENT_INSET - WorldData.TILE_SIZE / 2, 0, WorldData.TILE_ACCENT_INSET - WorldData.TILE_SIZE / 2);
    ...
    checkAdjacentTiles(world) {
        if (Main.screen instanceof RoomEditor) {
            return;
        }
        const tilePosition = this.tilePosition();
        const isGateOrSolid = (position) => (world.tiles.get(position) instanceof BasicTile || Gate.isGateAt(position, world));
    ...
    adjacentGates(world, x, y, direction) {
    ...
    checkPlayer(world) {
        ...
        const adjacentTile = tilePosition.add(Vector.unit((Directions.isVertical(this.direction))
    ...
    reflect() {
        const hitbox = Rectangle.fromBounds(RoomData.SIZE * WorldData.TILE_SIZE - this.hitbox.right, RoomData.SIZE * WorldData.TILE_SIZE - this.hitbox.left, this.hitbox.top, this.hitbox.bottom);
    ...
    static getGateAt(tilePosition, world) {
        const tileRect = Rectangle.square(tilePosition.x, tilePosition.y, 1).scale(WorldData.TILE_SIZE);
        return [...world.entities.possiblyIntersecting(tileRect)].find(e => e instanceof Gate && e.tilePosition().equals(tilePosition));
    }
    static deleteGateAt(tilePosition, world) {
        const gate = Gate.getGateAt(tilePosition, world);
        if (gate) {
            world.entities.delete(gate);
        }
    }
    static isGateAt(tilePosition, world) {
        return Gate.getGateAt(tilePosition, world) != undefined;
    }
    static attachedGates(tilePosition, world) {
        const attached = [];
    ...
    static destroyNonGateTile(position, world) {
