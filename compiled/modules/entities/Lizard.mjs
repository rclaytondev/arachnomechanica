...
export class Lizard extends Collideable {
    ...
    nextTurn = null;
    legPosition = 0;
    legDestination = LizardData.LEG_MAX;
    mouthAngle = 0;
    mouthDestination = LizardData.MAX_MOUTH_ANGLE;
    waitingTimer = -1;
    fireSpawner;
    ...
    displayGlowEffect(canvasIO) {
        ...
        GameUtils.glowCircle(0, LizardData.EYE_Y + LizardData.HEAD_OFFSET, LizardData.LIGHT_SIZE, LizardData.LIGHT_INTENSITY, canvasIO, LizardData.GLOW_COLOR.red, LizardData.GLOW_COLOR.green, LizardData.GLOW_COLOR.blue);
        canvasIO.ctx.restore();
    }
    getLegAngle(distance) {
        ...
        if (distance < distanceBefore + LizardData.LEG_ROTATION_START && jointBefore !== "head") {
            ...
            return GameUtils.lerpAngle(distance, distanceBefore, distanceBefore + LizardData.LEG_ROTATION_START, GameUtils.diagonalAngle(directionBefore, previousDirection), angleBefore);
        }
        else if (distance > distanceAfter - LizardData.LEG_ROTATION_END && jointAfter !== "tail") {
            return GameUtils.lerpAngle(distance, distanceAfter - LizardData.LEG_ROTATION_END, distanceAfter, angleBefore, GameUtils.diagonalAngle(directionBefore, jointAfter.direction));
        }
        else {
            return angleBefore;
        }
    ...
    legDisplaySegments(startDistance = 0) {
        const results = [];
        for (let i = 1; i * LizardData.LEG_SPACING < this.length; i++) {
            if (i * LizardData.LEG_SPACING < startDistance) {
                continue;
            }
    ...
    displayHead(canvasIO) {
        ...
        canvasIO.fillPoly(0, LizardData.HEAD_OFFSET, -LizardData.HEAD_WIDTH, LizardData.HEAD_HEIGHT + LizardData.HEAD_OFFSET, -mouthEnd.x, mouthEnd.y, 0, LizardData.HEAD_HEIGHT * 1.5 + LizardData.HEAD_OFFSET, mouthEnd.x, mouthEnd.y, LizardData.HEAD_WIDTH, LizardData.HEAD_HEIGHT + LizardData.HEAD_OFFSET);
    ...
    displayLookaheadRectangle(canvasIO) {
    ...
    updateMotion(world, canvasIO) {
        for (let i = 0; i < this.speed; i++) {
            this.updateMotion1Pixel(world, canvasIO);
        }
    }
    updateMotion1Pixel(world, canvasIO) {
    ...
    updateMouth() {
        if (this.waitingTimer < 0) {
            this.mouthAngle = GameUtils.moveTowards(this.mouthAngle, this.mouthDestination, (this.mouthAngle < this.mouthDestination) ? LizardData.MOUTH_SPEED_OPENING : LizardData.MOUTH_SPEED_CLOSING);
        }
        if (this.mouthAngle <= 0) {
            this.mouthDestination = LizardData.MAX_MOUTH_ANGLE;
        }
        if (this.mouthAngle >= LizardData.MAX_MOUTH_ANGLE) {
            this.mouthDestination = 0;
        }
    ...
    updateHeadAngle() {
        const minValue = ArrayUtils.minValue([this.headAngle, this.headAngle - 2 * Math.PI, this.headAngle + 2 * Math.PI], angle => MathUtils.dist(angle, this.targetHeadAngle));
    ...
    checkForPlayerTurns(world) {
        ...
        let nextTurn = null;
        if (player.bottom > this.position.y - LizardData.PLAYER_DETECTION_WIDTH / 2 &&
            player.top < this.position.y + LizardData.PLAYER_DETECTION_WIDTH / 2 &&
            !this.isObstructed(world, xDirection, LizardData.LOOKAHEAD_DISTANCE, Math.min(MathUtils.dist(lookaheadPoint.x, player.left), MathUtils.dist(lookaheadPoint.x, player.right)) - LizardData.LOOKAHEAD_DISTANCE)) {
            nextTurn = xDirection;
        }
        else if (player.right > this.position.x - LizardData.PLAYER_DETECTION_WIDTH / 2 &&
            player.left < this.position.x + LizardData.PLAYER_DETECTION_WIDTH / 2 &&
            !this.isObstructed(world, yDirection, LizardData.LOOKAHEAD_DISTANCE, Math.min(MathUtils.dist(lookaheadPoint.y, player.top), MathUtils.dist(lookaheadPoint.y, player.bottom)) - LizardData.LOOKAHEAD_DISTANCE)) {
            nextTurn = yDirection;
        }
    ...
    spawnBodyDamageParticles(newLength, world, canvasIO) {
    ...
    spawnDamageParticle(endpoint1, endpoint2, world, canvasIO) {
        const midpoint = endpoint1.add(endpoint2).divide(2);
        const velocity = new Vector(GameUtils.random(-LizardData.DAMAGE_PARTICLES.VELOCITY.X, LizardData.DAMAGE_PARTICLES.VELOCITY.X), GameUtils.random(LizardData.DAMAGE_PARTICLES.VELOCITY.Y.MIN, LizardData.DAMAGE_PARTICLES.VELOCITY.Y.MAX));
        const settings = {
    ...
    lookaheadPoint(direction = this.direction, distance = LizardData.LOOKAHEAD_DISTANCE) {
        return this.position.add(Vector.unit(direction).multiply(distance));
    }
    lookaheadRectangle(direction = this.direction, distance = LizardData.LOOKAHEAD_DISTANCE, length = 1, width = LizardData.LOOKAHEAD_WIDTH) {
        const point = this.lookaheadPoint(direction, distance);
        if (Directions.isHorizontal(direction)) {
            return Rectangle.fromDimensions(point.x - (direction === "left" ? length : 0), point.y - width / 2, length, width);
        }
        else {
            return Rectangle.fromDimensions(point.x - width / 2, point.y - (direction === "up" ? length : 0), width, length);
        }
    }
    isObstructed(world, direction = this.direction, distance = LizardData.LOOKAHEAD_DISTANCE, length = 1) {
    ...
    static segmentHitbox(point1, point2) {
        const HALF_HITBOX = Math.floor(LizardData.HITBOX_WIDTH / 2);
        if (point1.x === point2.x) {
            return Rectangle.fromBounds(point1.x - HALF_HITBOX, point1.x + HALF_HITBOX, Math.min(point1.y, point2.y) - HALF_HITBOX, Math.max(point1.y, point2.y) + HALF_HITBOX);
        }
        else {
            return Rectangle.fromBounds(Math.min(point1.x, point2.x) - HALF_HITBOX, Math.max(point1.x, point2.x) + HALF_HITBOX, point1.y - HALF_HITBOX, point1.y + HALF_HITBOX);
        }
    }
    hitboxes() {
    ...
    static spawn(tilePosition, world) {
        ...
        if (maxLength >= LizardData.MIN_LENGTH) {
            ...
            return world.addEntityIfEmpty(new Lizard(tilePosition.add(1 / 2, 1 / 2).multiply(WorldData.TILE_SIZE), direction, (GameUtils.randomInt(LizardData.MIN_LENGTH, length) + 1 / 2) * WorldData.TILE_SIZE, LizardData.SPEED));
            return true;
        }
        return false;
    }
    boundingBox() {
        return Rectangle.boundingBox(this.hitboxes().flatMap(r => [r.getCorner("top-left"), r.getCorner("top-right"), r.getCorner("bottom-left"), r.getCorner("bottom-right")]));
    }
    translate(amount) {
...
LoadingManager.onload(() => {
    EntitySpawner.registerEntityType((tileRegion, safeRegion, world) => {
        EntitySpawner.spawnEntities(tileRegion.area() / (RoomData.SIZE ** 2) * LizardData.LIZARDS_PER_ROOM, LizardData.SPAWN_EVENNESS, tileRegion, [EntitySpawner.spawnRequirements.replaceEmpty], Lizard.spawn, safeRegion, world);
    });
});
//# sourceMappingURL=Lizard.mjs.map