...
class BackgroundGear {
    ...
    color;
    image = null;
    constructor(position, size, teeth, innerRadiusRatio, speed, color, startAngle) {
    ...
    display(position, blur, canvasIO) {
        if (!this.image) {
    ...
    static isVisible(position, size, canvasIO) {
        return (position.x + size > 0 && position.x - size < canvasIO.canvas.width &&
            position.y + size > 0 && position.y - size < canvasIO.canvas.height);
    }
    intersects(gear, parallax) {
...
class GearLayer {
    ...
    blur;
    gears;
    constructor(parallax, blur, gears) {
        this.parallax = parallax;
        this.blur = blur;
        this.gears = gears;
    }
    display(cameraPosition, canvasIO) {
        canvasIO.ctx.save();
        for (const gear of this.gears) {
            const repeatedPosition = new Vector(cameraPosition.x + GameUtils.signedModularDistance(cameraPosition.x, gear.position.x, BackgroundData.BACKGROUND_REPEAT_SIZE), cameraPosition.y + GameUtils.signedModularDistance(cameraPosition.y, gear.position.y, BackgroundData.BACKGROUND_REPEAT_SIZE));
    ...
    static generate(info) {
        ...
        for (let i = 0; i < numGears; i++) {
            ...
            while (!spawned) {
                ...
                const gear = new BackgroundGear(position, GameUtils.random(info.minSize, info.maxSize), GameUtils.randomInt(info.minTeeth, info.maxTeeth), GameUtils.random(info.minInnerRadius, info.maxInnerRadius), GameUtils.random(info.minSpeed, info.maxSpeed) * (Math.random() < 0.5 ? 1 : -1), info.color, GameUtils.randomInt(0, 360));
...
export class GearsBackground extends Background {
    ...
    display(canvasIO, camera) {
        ...
        canvasIO.ctx.save();
        canvasIO.clipRect(translation.x, 0, LevelGeneratorData.WIDTH * RoomData.SIZE * WorldData.TILE_SIZE, canvasIO.canvas.height);
...
//# sourceMappingURL=GearsBackground.mjs.map