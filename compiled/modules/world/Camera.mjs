...
export class Camera {
    ...
    translation(canvasIO) {
        return new Vector(canvasIO.canvas.width / 2 - this.position.x, canvasIO.canvas.height / 2 - this.position.y);
    }
    applyTranslation(canvasIO) {
        const translation = this.translation(canvasIO);
        canvasIO.ctx.translate(translation.x, translation.y);
    }
    static visibleRegion(canvasIO, position, offscreenAmount) {
        return Rectangle.fromBounds(position.x - canvasIO.canvas.width / 2 - offscreenAmount, position.x + canvasIO.canvas.width / 2 + offscreenAmount, position.y - canvasIO.canvas.height / 2 - offscreenAmount, position.y + canvasIO.canvas.height / 2 + offscreenAmount);
    }
    visibleRegion(canvasIO, offscreenAmount) {
        return Camera.visibleRegion(canvasIO, this.position, offscreenAmount);
    }
    visibleTileRegion(canvasIO, offscreenTiles = 0) {
        const center = this.position.divide(WorldData.TILE_SIZE);
        return Rectangle.fromBounds(Math.floor(center.x - (canvasIO.canvas.width / 2 / WorldData.TILE_SIZE)) - offscreenTiles, Math.ceil(center.x + (canvasIO.canvas.width / 2 / WorldData.TILE_SIZE)) + offscreenTiles, Math.floor(center.y - (canvasIO.canvas.height / 2 / WorldData.TILE_SIZE)) - offscreenTiles, Math.ceil(center.y + (canvasIO.canvas.height / 2 / WorldData.TILE_SIZE)) + offscreenTiles);
    }
    static isCameraPositionValid(position, entities, canvasIO) {
    ...
    moveCameraIfValid(offset, entities, canvasIO) {
    ...
    update(target, entities, canvasIO) {
