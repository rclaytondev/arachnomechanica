...
export class LevelGenerator {
    ...
    possibleNextDirections(x, y) {
        if (this.path.length <= 1) {
            return [
                ...((x > 0) ? ["left"] : []),
                ...((x < LevelGeneratorData.WIDTH - 1) ? ["right"] : []),
                "down",
            ];
        }
    ...
    generateExitsOnPath() {
        for (const position of this.path) {
            ...
            for (const exit of exits) {
                if ((Directions.isHorizontal(exit) && Math.random() < LevelGeneratorData.MAIN_PATH_BRANCH_PROBABILITY_X) ||
    ...
    generateExits(position) {
        const exits = Directions.DIRECTIONS.filter(dir => (this.rooms.get(position.add(Vector.unit(dir)))?.exits.has(Directions.opposite[dir])));
        if (exits.length === 0) {
            return false;
        }
        ...
        for (const exit of otherExits) {
            ...
            if (((Directions.isHorizontal(exit) && Math.random() < LevelGeneratorData.OFF_PATH_BRANCH_PROBABILITY_X)
    ...
    generateRoom(roomPlaceholder) {
        const possibleRooms = MapUtils.groupBy(ROOMS.filter(r => (r.canSpawnWithExits(roomPlaceholder.exits)
            && r.isOrdinaryRoom())), r => Room.connectivity(r.traversability, roomPlaceholder.exits));
        while (possibleRooms.size > 0) {
            ...
            for (const connectedness of [...possibleRooms.keys()]) {
                ...
                possibleRooms.set(connectedness, group.filter(r => !SetUtils.isSubset(Room.filterTraversability(r.traversability, roomPlaceholder.exits).map(s => `${s.end}, ${s.start}`), Room.filterTraversability(room.traversability, roomPlaceholder.exits).map(s => `${s.end}, ${s.start}`))));
    ...
    attemptRoomSpawn(roomPlaceholder, room) {
    ...
    getRoomCandidate(roomsWithConnectivities) {
    ...
    // 	return result;
    // }
    getWeight(room, connectivity) {
        const generatability = room.getGeneratability();
        return (LevelGeneratorData.GENERATABILITY_MULTIPLIER * (1 - generatability)
            + LevelGeneratorData.CONNECTIVITY_MULTIPLIER * (1 - connectivity)
            + LevelGeneratorData.WEIGHT_BONUS) * (this.hasGenerated(room) ? LevelGeneratorData.DUPLICATE_PENALTY_MULTIPLIER : 1);
    }
    addRooms(world) {
        for (const position of this.levelRectangle().squares()) {
            ...
            if (roomPlaceholder) {
                ...
                const rectangle = Rectangle.square(this.position.x + position.x * RoomData.SIZE, this.position.y + position.y * RoomData.SIZE, RoomData.SIZE);
    ...
    addBorders(world) {
        ...
        fillSolidRect(this.position.x - LevelGeneratorData.BORDER_X, this.position.y - LevelGeneratorData.BORDER_Y, LevelGeneratorData.WIDTH * RoomData.SIZE + LevelGeneratorData.BORDER_X, LevelGeneratorData.BORDER_Y);
        fillSolidRect(this.position.x - LevelGeneratorData.BORDER_X, this.position.y, LevelGeneratorData.BORDER_X, LevelGeneratorData.HEIGHT * RoomData.SIZE);
        fillSolidRect(this.position.x + LevelGeneratorData.WIDTH * RoomData.SIZE, this.position.y - LevelGeneratorData.BORDER_Y, LevelGeneratorData.BORDER_X, LevelGeneratorData.HEIGHT * RoomData.SIZE + LevelGeneratorData.BORDER_Y);
        fillSolidRect(this.position.x - LevelGeneratorData.BORDER_X, this.position.y + LevelGeneratorData.HEIGHT * RoomData.SIZE, LevelGeneratorData.WIDTH * RoomData.SIZE + 2 * LevelGeneratorData.BORDER_X, LevelGeneratorData.BORDER_Y);
    }
    isInBounds(position) {
        return (0 <= position.x && position.x < LevelGeneratorData.WIDTH &&
            0 <= position.y && position.y < LevelGeneratorData.HEIGHT);
    }
    isEdgeInBounds(position, direction) {
        return this.isInBounds(position) && this.isInBounds(position.add(Vector.unit(direction)));
    }
    setConnected(roomPosition, direction, connected) {
    ...
    isConnected() {
        ...
        for (const backwards of [true, false]) {
            const reachable = GameUtils.reachableNodes(startState, (state) => this.neighbors(state, backwards).filter(n => this.isEdgeInBounds(n.position, n.exit)), v => v.normalize().toString(true));
    ...
    neighbors(state, backwards = false) {
        if (!state.position) {
        ...
        for (const position of positions) {
            ...
            for (const { start, end } of room.room.traversability) {
                if (!room.exits.has(start.exit) || !room.exits.has(end.exit)) {
                    continue;
                }
    ...
    visualize(canvasIO, pauseDebugger = true) {
    ...
    visualizeExits(canvasIO, room, position) {
        position = position.multiply(DEBUG_SETTINGS.GENERATOR_VISUALIZATION.GRID_SIZE);
        canvasIO.ctx.fillStyle = "black";
        canvasIO.ctx.fillRect(position.x, position.y, room.exits.has("up") ? DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE : DEBUG_SETTINGS.GENERATOR_VISUALIZATION.GRID_SIZE, DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE);
        canvasIO.ctx.fillRect(position.x, position.y, DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE, room.exits.has("left") ? DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE : DEBUG_SETTINGS.GENERATOR_VISUALIZATION.GRID_SIZE);
        canvasIO.ctx.fillRect(position.x, position.y + DEBUG_SETTINGS.GENERATOR_VISUALIZATION.GRID_SIZE - DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE, room.exits.has("down") ? DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE : DEBUG_SETTINGS.GENERATOR_VISUALIZATION.GRID_SIZE, DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE);
        canvasIO.ctx.fillRect(position.x + DEBUG_SETTINGS.GENERATOR_VISUALIZATION.GRID_SIZE - DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE, position.y, DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE, room.exits.has("right") ? DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE : DEBUG_SETTINGS.GENERATOR_VISUALIZATION.GRID_SIZE);
        canvasIO.ctx.fillRect(position.x + DEBUG_SETTINGS.GENERATOR_VISUALIZATION.GRID_SIZE - DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE, position.y + DEBUG_SETTINGS.GENERATOR_VISUALIZATION.GRID_SIZE - DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE, DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE, DEBUG_SETTINGS.GENERATOR_VISUALIZATION.BORDER_SIZE);
    }
    visualizeRoom(canvasIO, roomPlaceholder, position) {
        for (const tilePosition of Rectangle.square(0, 0, RoomData.SIZE).squares()) {
            ...
            if (tile instanceof BasicTile || tile === Platform.PLATFORM || (exitTile !== "none" && !roomPlaceholder.exits.has(exitTile))) {
                canvasIO.ctx.fillStyle = "black";
            }
            else {
                continue;
            }
            canvasIO.fillSquare(DEBUG_SETTINGS.GENERATOR_VISUALIZATION.GRID_SIZE * (position.x + tilePosition.x / RoomData.SIZE), DEBUG_SETTINGS.GENERATOR_VISUALIZATION.GRID_SIZE * (position.y + tilePosition.y / RoomData.SIZE), DEBUG_SETTINGS.GENERATOR_VISUALIZATION.GRID_SIZE / RoomData.SIZE);
        }
    }
    static roomFrequencies(numTrials) {
...
//# sourceMappingURL=LevelGenerator.mjs.map